...
def _small_m_direct_host_barrier_reset_enabled() -> bool:
    ...
    scalars.  The fork removed the host clears in production (5539bb2,
    "Remove redundant W4A16 barrier clears from graph replay") as two saved
    captured launches per replay; upstream kept them as the historical
    default.  Default here is OFF to preserve the fork's measured behavior;
    flip B12X_W4A16_SMALL_M_HOST_BARRIER_RESET=1 to restore the host clears.
    """
